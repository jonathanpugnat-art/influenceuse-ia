...
- **Sources réelles** (avec `APIFY_TOKEN` configuré) :
  - **TikTok** : top hashtags du Creative Center via l'actor `khadinakbar/tiktok-trending-hashtags-scraper` (pay-per-use ; override `APIFY_TIKTOK_ACTOR` pour l'ancien `scrapeengine/…` loué). Configurable par pays (`APIFY_TIKTOK_COUNTRY`) et fenêtre 7/30/120 jours (`APIFY_TIKTOK_PERIOD`). Les `industry_info` Apify sont mappées sur nos `Niche` Prisma, les `video_views` log-normalisées en `growthScore` 0–100.
