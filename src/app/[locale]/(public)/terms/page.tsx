...
export default async function TermsPage({ params }: Props) {
  ...
  const bannedItems = t.raw("s5Items") as string[];
  ...
  return (
    <LegalPageFrame locale={locale}>
      ...
      <section className="space-y-3">
        ...
        <p className="leading-relaxed">{t("s3Body")}</p>
      </section>

      <section className="space-y-3">
        <h2 className="text-xl font-semibold text-white">{t("s4Title")}</h2>
        <p className="leading-relaxed">{t.rich("s4Body", { email: emailTag })}</p>
      </section>

      <section className="space-y-3">
        <h2 className="text-xl font-semibold text-white">{t("s5Title")}</h2>
        <p className="leading-relaxed">{t("s5Intro")}</p>
        <ul className="list-inside list-disc space-y-1 text-slate-300">
          {bannedItems.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
        <p className="leading-relaxed">{t("s5Breach")}</p>
      </section>
      ...
      <section className="space-y-3">
        ...
        <p className="leading-relaxed">
          <strong>{t("s6Transparency")}</strong>
        </p>
      </section>
      ...
      <section className="space-y-3">
        ...
        <p className="leading-relaxed">{t("s7Refund")}</p>
      </section>
      ...
      <section className="space-y-3">
        <h2 className="text-xl font-semibold text-white">{t("s10Title")}</h2>
        <p className="leading-relaxed">{t("s10Body")}</p>
      </section>

      <section className="space-y-3">
        <h2 className="text-xl font-semibold text-white">{t("s11Title")}</h2>
        <p className="leading-relaxed">{t.rich("s11Body", { email: emailTag })}</p>
      </section>
    </LegalPageFrame>
  );
}
